...
class CFG:
    ...
    # Exit-loss weights (lambda_i) annealing (length=4, exits shallow->deep)
    lambda_start: Tuple[float, ...] = (1.0, 0.7, 0.4, 0.1)
    lambda_end:   Tuple[float, ...] = (0.2, 0.5, 0.8, 1.0)

    # SoftCGT shaping (GT-probability formulation):
    # s_e(i) = p_e(y_i|x_i)
    # Residual gate (SoftCGT): r_e(i) = 1 - sigmoid( (s_e(i) - tau_train) * sigmoid_scale )
    tau_train: float = 0.9
...
@torch.no_grad()
def gt_confidence(logits: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    # s_e(i) = p_e(y_i|x_i), ground-truth probability. Shape [B].
    probs = F.softmax(logits, dim=1)
    return probs.gather(1, y.view(-1, 1)).squeeze(1)


def softcgt_lambda_anneal_loss(
    ...
    # SoftCGT (GT-probability formulation + residual gating):
    #   s_e(i) = p_e(y_i|x_i)   (GT-class prob)
    #   r_e(i) = 1 - sigmoid( (s_e(i) - tau_train) * sigmoid_scale )
    #            (high s_e -> small r_e -> attenuate deeper gradients)

    #   elig_1(i)=1
    #   elig_e(i)= Π_{k<e} r_k(i)
    # Weights:
    ...
    s_list = [gt_confidence(logits_list_shallow2deep[e], y) for e in range(E)]
    # Residual gating (SoftCGT): r_e = 1 - sigmoid((s_e - tau_train) * sigmoid_scale)
    r_list = [1.0 - torch.sigmoid((s - tau_train) * sigmoid_scale) for s in s_list]
    r_list = [r.clamp(0.0, 1.0) for r in r_list]

    elig = torch.ones(B, device=device)
    eligs = [elig]
    for e in range(1, E):
        elig = elig * r_list[e - 1]
        eligs.append(elig)
    ...
    stats = {
        "s_mean": torch.stack([s.mean() for s in s_list]).detach(),
        "r_mean": torch.stack([r.mean() for r in r_list]).detach(),
        "elig_mean": torch.stack([el.mean() for el in eligs]).detach(),
        "w_mean": W.mean(dim=0).detach(),
    }
    return total, stats
...
def make_loaders(cfg: CFG):
    ...
    # Stronger augmentation (keeps hard labels, plays nicely with SoftCGT gating):
    train_tf = T.Compose([
...
def train_one_epoch(model, loader, optimizer, cfg: CFG, epoch: int, lambda_vec: torch.Tensor):
    ...
    for it, (x, y) in enumerate(loader):
        ...
        loss, stats = softcgt_lambda_anneal_loss(
            outs, y,
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        if acc > best_acc:
            ...
            torch.save(
                ...
                f"checkpoints/best_resnet18_att_softcgt_cifar100_tau_lambda_anneal_strongaug_tau_{cfg.tau_train}.pth",
            )

    print(f"Done. Best acc={best_acc*100:.2f}%")
    print(f"Saved: checkpoints/best_resnet18_att_softcgt_cifar100_tau_lambda_anneal_strongaug_tau_{cfg.tau_train}.pth")
